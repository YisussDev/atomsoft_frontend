  ...
  private static createFileRepository(pathEntities: string, pathEntityExample: string, namePrincipalModule: string, nameSecondModule: string, nameFileClass: string) {
    fs.readFile(
      ...
      (err: any, text: string) => {
        ...
        if (!fs.existsSync(`${pathEntities}/${namePrincipalModule}/${nameSecondModule}.repository.ts`)) fs.writeFileSync(`${pathEntities}/${namePrincipalModule}/${nameSecondModule}.repository.ts`, data);
      }
    );
  }
