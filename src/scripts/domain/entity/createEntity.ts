...
export class CreateEntity extends OperationsModel {
  ...
  private static createFileEntity(
    ...
    fs.readFile(
      ...
      "utf8",
      (err: any, text: string) => {
        const data = new Uint8Array(buffer.Buffer.from(this.replaceNamesInFile(text, namePrincipalModule, nameSecondModule, nameFileClass + 'Entity')));
        if (!fs.existsSync(`${pathEntities}/${namePrincipalModule}/${nameSecondModule}.entity.ts`)) fs.writeFileSync(`${pathEntities}/${namePrincipalModule}/${nameSecondModule}.entity.ts`, data);
      }
    );
  }
