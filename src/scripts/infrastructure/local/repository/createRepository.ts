...
export class CreateRepository extends OperationsModel {
  ...
  static constructRepository(pathEntities: string, pathEntitiesExample: string) {
  ...
  private static createFileImplementation(
    ...
    fs.readFile(
      ...
      (err: any, text: string) => {
        ...
        if (!fs.existsSync(`${pathEntities}/${namePrincipalModule}/${nameSecondModule}-local.repository.ts`)) fs.writeFileSync(`${pathEntities}/${namePrincipalModule}/${nameSecondModule}-local.repository.ts`, data);
      }
    );
  }
