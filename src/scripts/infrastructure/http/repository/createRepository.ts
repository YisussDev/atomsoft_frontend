  ...
  private static createFileImplementation(
    ...
    fs.readFile(
      ...
      (err: any, text: string) => {
        ...
        if (!fs.existsSync(`${pathEntities}/${namePrincipalModule}/${nameSecondModule}-http.repository.ts`)) fs.writeFileSync(`${pathEntities}/${namePrincipalModule}/${nameSecondModule}-http.repository.ts`, data);
      }
    );
  }
