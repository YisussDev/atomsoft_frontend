  ...
  private static createFileModel(
    ...
    fs.readFile(
      ...
      (err: any, text: string) => {
        ...
        if (!fs.existsSync(`${pathEntities}/${namePrincipalModule}/${nameSecondModule}-http-model.ts`)) fs.writeFileSync(`${pathEntities}/${namePrincipalModule}/${nameSecondModule}-http-model.ts`, data);
      }
    );
  }
