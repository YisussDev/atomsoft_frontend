...
export class OperationsModel {
  ...
  protected static replaceNamesInFile(textFile: string, namePrincipalModule: string, nameSecondModule: string, nameModule: string, nameUseCaseSnakeCase?: string, nameUseCaseCamelCase?: string): string {
    ...
    if (nameUseCaseCamelCase) {
      textFile = textFile.replace(/{{nameUseCaseCamelCase}}/g, nameUseCaseCamelCase);
    }
    if (nameUseCaseSnakeCase) {
      textFile = textFile.replace(/{{nameUseCaseSnakeCase}}/g, nameUseCaseSnakeCase);
    }
    return textFile;
  }

  protected static replaceNamesInFileUseCase(
    textFile: string,
  ...
  protected static createFolderUseCase(path: string, nameFolder: string, nameSecondModule?: string): void {
    if (!fs.existsSync(path + `/${nameFolder}`)) fs.mkdirSync(path + `/${nameFolder}`);
    if (nameSecondModule) {
      if ((!fs.existsSync(path + `/${nameFolder}/${nameSecondModule}`))) fs.mkdirSync(path + `/${nameFolder}/${nameSecondModule}`);
    }
