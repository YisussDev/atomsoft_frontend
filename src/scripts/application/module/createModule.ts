...
export class CreateModule extends OperationsModel {
  ...
  private static createFileModule(
    ...
    fs.readFile(
      ...
      (err: any, text: string) => {
        ...
        if (
          !fs.existsSync(`${pathEntities}/${namePrincipalModule}/${nameSecondModule}/${nameUseCaseSnakeCase}-${namePrincipalModule}-use-case.module.ts`)
        ) fs.writeFileSync(`${pathEntities}/${namePrincipalModule}/${nameSecondModule}/${nameUseCaseSnakeCase}-${namePrincipalModule}-use-case.module.ts`, data);
      }
    );
  }
