...
export class FilesService {
  ...
  public async convertToBase664(file: File, limit: number = 1): Promise<FileInterface> {
  ...
  public downloadFileLocal(fileName: string, filePath: string): void {

    this._http.get(filePath, {responseType: 'blob'}).subscribe((blob) => {
  ...
  public async downloadDiv(nameFile: string, idDiv: string): Promise<void> {
    try {
      ...
      if (element) {
        ...
        html2canvas(element).then(canvas => {
          // this.pdfService.generateVoucher(canvas);
  ...
  public async generateCanva(nameFile: string, idDiv: string): Promise<HTMLCanvasElement | null> {
    try {
      const element = document.getElementById(idDiv);
      if (!element) {
        console.warn(`Elemento con id "${idDiv}" no encontrado.`);
        return null;
      }
