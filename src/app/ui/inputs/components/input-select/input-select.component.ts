...
export class InputSelectComponent extends InputSchema implements OnInit, OnDestroy {
  ...
  @Input() public keyToShowSecondControl!: string;
  @Input() public optionsSelect: any[] = [];
  ...
  public override initInput(): void {
    ...
    if((typeof this.control.value == "string" || typeof this.control.value == "number") && (this.control.value.toString().length > 0)){
      this.inputValue = this.control.value.toString();
